...
//			}
		...
		public void onPageSelected(int position) {
			...
			if (lastPointPosition == 0) {
				...
				if (currentPointIndex == imagesSize - 1) {
					currentPointIndex = 0;
				} else {
					currentPointIndex += 1;
				}
				...
				if (currentPointIndex == 0) {
					currentPointIndex = imagesSize - 1;
				} else {
					currentPointIndex -= 1;
				}

			}
			if (dots.size() < images.size() && lastPointIndex == currentPointIndex) {
				dots.get(1).setImageResource(R.drawable.face_float_icon_on);
			} else {
				dots.get(lastPointIndex).setImageResource(R.drawable.face_float_icon_on);
			}
			dots.get(currentPointIndex).setImageResource(R.drawable.face_float_icon);
