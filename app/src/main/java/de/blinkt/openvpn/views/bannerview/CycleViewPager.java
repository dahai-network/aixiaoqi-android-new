package de.blinkt.openvpn.views.bannerview;
...
public class CycleViewPager extends LinearLayout implements OnPageChangeListener {
	...
	private List<IndexBannerEntity> infos;
...
//		viewPager.setOffscreenPageLimit(3);
	...
	private void setIndicator(int selectedPosition) {
		for (int i = 0; i < indicators.length; i++) {
			indicators[i]
					.setBackgroundResource(R.drawable.face_float_icon_on);
		}
		if (indicators.length > selectedPosition)
			indicators[selectedPosition]
					.setBackgroundResource(R.drawable.face_float_icon);
	}
