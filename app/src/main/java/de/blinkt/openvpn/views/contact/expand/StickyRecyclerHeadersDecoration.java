package de.blinkt.openvpn.views.contact.expand;
...
public class StickyRecyclerHeadersDecoration extends RecyclerView.ItemDecoration {
	...
	public int findHeaderPositionUnder(int x, int y) {
		SparseArray<Rect> localMHeaderRects = mHeaderRects;
		int mHearderRectsInt = localMHeaderRects.size();
		for (int i = 0; i < mHearderRectsInt; i++) {
			Rect rect = localMHeaderRects.get(localMHeaderRects.keyAt(i));
			if (rect.contains(x, y)) {
				return localMHeaderRects.keyAt(i);
			}
		}
		return -1;
	}
