...
public class IntentWrapper {
    protected static final int NETWORK =97;
    //Android 6.0+ Doze 模式
...
//                return new ArrayList<>();
            ...
            if (Build.VERSION.SDK_INT >= 24) {
                ...
                PowerManager pm = (PowerManager)  ICSOpenVPNApplication.getInstance().getSystemService(Context.POWER_SERVICE);
                boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations( ICSOpenVPNApplication.getInstance().getPackageName());
                if (!ignoringBatteryOptimizations) {
                    Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                    dozeIntent.setData(Uri.parse("package:" +  ICSOpenVPNApplication.getInstance().getPackageName()));
                    sIntentWrapperList.add(new IntentWrapper(dozeIntent, DOZE));
                }
            }
            Intent netWorkIntent = new Intent(Settings.ACTION_WIFI_SETTINGS);
            sIntentWrapperList.add(new IntentWrapper(netWorkIntent, NETWORK));
            //华为 自启管理
...
//            oppoIntent.setComponent(new ComponentName("com.coloros.safecenter", "com.coloros.safecenter.permission.startup.StartupAppListActivity"));
    ...
    protected static String getApplicationName() {
        if (sApplicationName == null) {
            ...
            ApplicationInfo ai;
            try {
                pm =  ICSOpenVPNApplication.getInstance().getPackageManager();
                ai = pm.getApplicationInfo( ICSOpenVPNApplication.getInstance().getPackageName(), 0);
                sApplicationName = pm.getApplicationLabel(ai).toString();
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
                sApplicationName =  ICSOpenVPNApplication.getInstance().getPackageName();
            }
        }
     ...
     * 处理白名单
     */
    public static void whiteListMatters(final Activity activity, String reason) {
            ...
            switch (iw.type) {
                case NETWORK:
                    new AlertDialog.Builder(activity)
                            .setCancelable(false)
                            .setTitle("在休眠状态下保持WLAN连接")
                            .setMessage("点击高级设置，进入高级设置界面。\n" +
                                    "点击在休眠状态下保持WLAN连接，选择始终。" )
                            .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface d, int w) {iw.startActivity(activity);}
                            })
                            .show();
                    break;
                case DOZE:
    ...
    protected boolean doesActivityExists() {
        PackageManager pm =  ICSOpenVPNApplication.getInstance().getPackageManager();
    ...
    protected void startActivity(Activity activity) {
        try {activity.startActivity(intent);} catch (Exception e) {e.printStackTrace();}
    }
}
