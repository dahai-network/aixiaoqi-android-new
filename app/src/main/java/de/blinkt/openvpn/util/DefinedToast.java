 ...
 * @author kim
 */
public class DefinedToast implements IToast {
     ...
     * 维护toast的队列
     */
    private static BlockingQueue<DefinedToast> mQueue = new LinkedBlockingQueue<DefinedToast>();
    ...
    public static IToast makeText(Context context, String text, long duration) {
        return new DefinedToast(context).setText(text).setDuration(duration)
    ...
    private static void activeQueue() {
        DefinedToast miuiToast = mQueue.peek();
        if (miuiToast == null) {
