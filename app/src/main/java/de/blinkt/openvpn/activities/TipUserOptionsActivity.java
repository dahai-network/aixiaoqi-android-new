...
public class TipUserOptionsActivity extends BaseActivity implements InterfaceCallback {
	...
	@BindView(R.id.callCommingSwitchView)
	SwitchView callCommingSwitchView;
	@BindView(R.id.smsSwitchView)
	SwitchView smsSwitchView;
	@BindView(R.id.qqSwitchView)
	SwitchView qqSwitchView;
	@BindView(R.id.weixinSwitchView)
	SwitchView weixinSwitchView;
	...
	private boolean isCanSendBluetooth;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_tip_user_options);
	...
	private void initTitle() {
		...
		hasLeftViewTitle(R.string.message_remind, 0);
		callCommingSwitchView.setOpened(utils.readInt(Constant.COMING_TEL_REMIND) == 1);
		smsSwitchView.setOpened(utils.readInt(Constant.MESSAGE_REMIND) == 1);
		qqSwitchView.setOpened(utils.readInt(Constant.QQ_REMIND) == 1);
		weixinSwitchView.setOpened(utils.readInt(Constant.WEIXIN_REMIND) == 1);
	}
...
//		}
	...
	public void rightComplete(int cmdType, CommonHttp object) {
		if (cmdType == HttpConfigUrl.COMTYPE_UPLOAD_REMIND_CONFIG) {
			if (object.getStatus() == 1) {
				if (isCanSendBluetooth) {
					reportSwitchState(callCommingSwitchView, Constant.COMING_TEL_REMIND);
					reportSwitchState(smsSwitchView, Constant.MESSAGE_REMIND);
					reportSwitchState(qqSwitchView, Constant.QQ_REMIND);
					reportSwitchState(weixinSwitchView, Constant.WEIXIN_REMIND);
					//发送蓝牙消息打开/关闭消息通知
					sendBlueEnable();
					finish();
					isCanSendBluetooth = false;
				}
			} else {
	...
	public void reportSwitchState(SwitchView switchView, String type) {
	...
	private void sendBlueEnable() {
		...
		String blueStr = "88800303000" + enableStr;
...
//		);
	...
	public void setNoClick(boolean noClick) {
		callCommingSwitchView.setNoClick(noClick);
		smsSwitchView.setNoClick(noClick);
		qqSwitchView.setNoClick(noClick);
		weixinSwitchView.setNoClick(noClick);
	}
}
