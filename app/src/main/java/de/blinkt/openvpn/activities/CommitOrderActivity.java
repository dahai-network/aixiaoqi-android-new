			...
			case R.id.sureTextView:
				...
				if (weixinPayCheckBox.isChecked()) {
					if (isWXAppInstalledAndSupported()) {
						...
						createHttpRequest(HttpConfigUrl.COMTYPE_CREATE_ORDER, bean.getPackageId(), packetCount + "", WEIXIN_PAY_METHOD + "");
					} else {
					...
					//友盟方法统计
					MobclickAgent.onEvent(this, CLICKSUREPAGMENT, map);
					createHttpRequest(HttpConfigUrl.COMTYPE_CREATE_ORDER, bean.getPackageId(), packetCount + "", ALI_PAY_METHOD + "");
				} else {
					map.put("type", BALANCE_PAY_METHOD + "");
					//友盟方法统计
					MobclickAgent.onEvent(this, CLICKSUREPAGMENT, map);
					createHttpRequest(HttpConfigUrl.COMTYPE_CREATE_ORDER, bean.getPackageId() + "", packetCount + "", BALANCE_PAY_METHOD + "");
				}
				break;
		}
	}
