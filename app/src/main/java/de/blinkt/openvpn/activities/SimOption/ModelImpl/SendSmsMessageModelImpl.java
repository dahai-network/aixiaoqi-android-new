package de.blinkt.openvpn.activities.SimOption.ModelImpl;

import de.blinkt.openvpn.activities.NetModelBaseImpl;
import de.blinkt.openvpn.activities.SimOption.Model.SendSmsMessageModel;
import de.blinkt.openvpn.constant.HttpConfigUrl;

/**
 * Created by Administrator on 2017/6/15 0015.
 */

public class SendSmsMessageModelImpl extends NetModelBaseImpl implements SendSmsMessageModel {
    public SendSmsMessageModelImpl(OnLoadFinishListener onLoadFinishListener){
        super(onLoadFinishListener);
    }
    @Override
    public void requestSendSmsMessage(String phoneNumber, String content) {
        createHttpRequest(HttpConfigUrl.COMTYPE_SEND_SMS_MESSAGE, phoneNumber, content);
    }
}
